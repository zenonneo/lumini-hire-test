...
namespace Elastic
{
    public class DataIndexer
    {
        ...
        public async Task<IndexResult> IndexRecipesFromFile(string fileName, bool deleteIndexIfExists, string index = null)
        {
            ...
            Dtos.Recipe[] mappedCollection = await ParseJsonFile(fileName);
        ...
        private async Task<IndexResult> IndexDocuments(Dtos.Recipe[] mappedCollection, string index)
        {
        ...
        private async Task<Dtos.Recipe[]> ParseJsonFile(string fileName)
        {
            using (FileStream fs = new FileStream(Path.Combine(contentRootPath, fileName), FileMode.Open))
            {
                using (StreamReader reader = new StreamReader(fs))
                {
                    ...
                    Dtos.Recipe[] mappedCollection = JsonConvert.DeserializeObject<Dtos.Recipe[]>(rawJsonCollection, new JsonSerializerSettings
                    {
